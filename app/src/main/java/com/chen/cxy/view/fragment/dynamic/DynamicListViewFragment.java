    ...
    private void showList(List<Map<String,Object>> list){
        if(adapter == null){
            adapter = new SimpleAdapter(context, list, R.layout.dynamic_list_item,
                    new String[]{"portrait_iv","nickname_iv","address_tv","content_tv","content_iv","like_count"},
                    new int []{R.id.portrait_iv,R.id.nickname_iv,R.id.address_tv,R.id.content_tv,R.id.content_iv,R.id.like_count});
        }else{
            this.list = list;
